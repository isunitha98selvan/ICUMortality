...
def add_age_to_icustays(stays):
    doa = pd.to_datetime(stays['INTIME']).dt.date
    dob  = pd.to_datetime(stays['DOB']).dt.date
    x = doa.subtract(dob)
    age =[]
    for i in x.values:
        temp = i /np.timedelta64(1, 'D').astype(int)
        # print(str(temp).split())
        age.append(int(str(temp).split()[0]) / 365)
    stays['AGE'] = pd.DataFrame(age)
